	...
	public function action_gettweets(){
			...
			foreach ($results->statuses as $key => $result) {
				...
				if(Tweet::where('tweet_id_str','=',$result->id_str)->first() == null){
					...
					$tweet->content="<div class='testing col-md-12 row'><img class='twitter-prof-pic' src='".$result->user->profile_image_url."'/>" . "		" ."<p class='twitter-tweet-content'>".$result->text ."</br><span>".$result->created_at."</span></p></div>";
					$tweet->rep_id=$rep_id;
					// $tweet->rep_id;
					$tweet->save();
				}
