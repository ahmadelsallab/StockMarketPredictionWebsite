...
for x in Stocks.objects.filter().exclude(stock=None).exclude(stock='none').exclude(stock='').exclude(stock='تاسي').order_by('-id'):
    line = x.stock_id
    stock_name = x.stock
    ...
    tweetes_to_render = Opinion.objects.filter(stock=line,stocksprices_id__isnull=True).order_by('-created_at');
