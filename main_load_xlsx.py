...
# Set if ready train and test are to be loaded from XLSX file 
LOAD_TRAIN_TEST_FROM_XLSX_FILE = True

# Set if dataset is to be loaded from XLSX file 
LOAD_DATASET_FROM_XLSX_FILE = False
...
LOAD_FEATURES = False
LOAD_LABELS = False
# Set to true if features are to be exported
DUMP_FEATURES = False
...
if(DATASET_BUILDER):
    ...
    xlsxManualLabelsFileName = ".\\DatasetBuilder\\Output\\ManualLabels_tasi"
    ...
    elif LOAD_DATASET_FROM_XLSX_FILE:
    ...
    elif LOAD_TRAIN_TEST_FROM_XLSX_FILE:
        # Initialize the DatasetBuilder from serialization file
        datasetBuilder = DatasetBuilder(configFileDatasetBuilder, [], datasetSerializationFile)
...
if(LANGUAGE_MODEL):
    ...
    if LOAD_LANGUAGE_MODEL:
        ...
        if MERGE_BI_GRAM:
            relLanguageModel.NGram = 2
            relLanguageModel.BuildLanguageModel()
        if MERGE_TRI_GRAM:
            relLanguageModel.NGram = 3
            relLanguageModel.BuildLanguageModel()
        ...
        if MERGE_BI_GRAM:
            irrelLanguageModel .NGram = 2
            irrelLanguageModel .BuildLanguageModel()
        if MERGE_TRI_GRAM:
            irrelLanguageModel .NGram = 3
            irrelLanguageModel .BuildLanguageModel()            
